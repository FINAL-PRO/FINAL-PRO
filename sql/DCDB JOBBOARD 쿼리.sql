...
-- 게시판  + 구인구직테이블 조회
SELECT JB.NO, TITLE, NICKNAME, WRITEDATE, STARTDATE, ENDDATE, FLOOR(ENDDATE - SYSDATE) "DDAY",
...
JOIN DC_CODE_TB CD4 ON(CD4.ID = B.STATUS)
WHERE B.STATUS != 'STATUS002'
AND NICKNAME = 'adminNick'
ORDER BY JB.NO DESC;

commit;
--  FLOOR(ENDDATE - SYSDATE) "DDAY", 
-- 게시물 상세조회
SELECT JB.NO, TITLE, NICKNAME, CONTENT, WRITEDATE, STARTDATE, ENDDATE, 
              FLOOR(ENDDATE - SYSDATE) "DDAY",
              LOCATION, STARTJOB, ENDJOB, SALARY, 
              CD1.VALUE  "JOBTYPE",
              CD2.VALUE "JOBTYPE", VIEWCOUNT,
              CD3.VALUE "TYPE",
              CD4.VALUE "STATUS"
...
-- 업데이트
UPDATE DC_BOARD_TB B 
SET  
TITLE = '바꿉니다. 제목',
CONTENT = '바꿉니다 내용'
WHERE B.NO = 26;

SELECT * FROM DC_BOARD_TB B
WHERE B.NO = 27;

SELECT MIN,
CASE WHEN MIN > 0 THEN HOUR - 1 end "HOUR"
FROM (
SELECT SUBSTR(ENDTIME, 1, 2) - SUBSTR(STARTTIME, 1, 2) HOUR, SUBSTR(ENDTIME, 4, 2) - SUBSTR(STARTTIME, 4, 2) MIN
FROM DC_BOARD_JOB_TB     
WHERE BOARDNO = 27
