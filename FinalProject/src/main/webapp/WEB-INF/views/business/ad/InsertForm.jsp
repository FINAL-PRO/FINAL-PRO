						...
						<div class="dc-content-box">
							...
							<table class="table">
								<tr>
									<td>광고타입</td>
									<td>
										<select class="form-control" name="adType" id="adType" <c:if test="${!empty ad}">disabled</c:if>>
											<c:if test="${!empty ad}">
								...
								<tr>
									<td>광고물타입</td>
									<td>
										<select class="form-control" name="adContentType" id="adContentType" <c:if test="${!empty ad}">disabled</c:if>>
								...
								<tr>
									...
									<td>
										<div class="form-inline">
											<select class="form-control" name="pageNo" id="" <c:if test="${!empty ad}">disabled</c:if>>
												<c:forEach items="${adPageList}" var="page">
													<option value="${page.no}">${page.name}</option>
												</c:forEach>
											</select>
											<select class="form-control" name="sectionNo" id="" <c:if test="${!empty ad}">disabled</c:if>>
												<c:forEach items="${adSectionList}" var="section">
													<option value="${section.no}">${section.name}</option>
												</c:forEach>
											</select>
											<select class="form-control" name="locationNo" id="" <c:if test="${!empty ad}">disabled</c:if>>
								...
								<tr>
									<td>시작일</td>
									<td>
										<input class="form-control" name="startDate" type="date" required="required" 
								...
								<tr>
									<td>종료일</td>
									<td>
										<input class="form-control" name="endDate" type="date" required="required" 
								...
								<tr>
									...
									<td>
										<c:if test="${empty ad }">
											<input class="form-control" name="adContentPath" type="file" required="required" <c:if test="${!empty ad}">disabled</c:if>/>
										</c:if>
										<c:if test="${!empty ad }">
								...
								<tr>
									...
									<td>
										<div class="form-inline">
											<input class="form-control" name="landingPage" type="text" required="required" 
											value="${ad.landingPage }"/>
											<button class="btn dc-btn" type="button" onclick="landingCheck();">랜딩확인</button>
										</div>
									</td>
								</tr>
								<tr>
									<td colspan="2">
										<c:if test="${empty ad}">
											<button class="btn dc-btn" type="submit">광고 신청</button>
										</c:if>
										<c:if test="${!empty ad}">
											<button class="btn dc-btn" type="submit">광고 수정</button>
										</c:if>
